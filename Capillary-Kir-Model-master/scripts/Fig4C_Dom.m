...
prompt = {'Enter Patch Experiment (ctrl, dep, vsp)'};
dlgtitle = 'Input';
dims = [1 35];
definput = {'ctrl'};
PATCH_CONDITION_STRING = inputdlg(prompt,dlgtitle,dims,definput);
...
Gkirbar = [];
if strcmp(PATCH_CONDITION_STRING, 'ctrl')
%------ CONTROL kir2.1 patch dataset distribution parameters
    Gkirbar = mu_control + sigma_control*randn(1,tot_cell_number);
end
%------ DEPRIVED kir2.1 patch dataset  distribution parameters
if strcmp(PATCH_CONDITION_STRING, 'dep')
    Gkirbar = mu_deprived + sigma_deprived*randn(1,tot_cell_number);
end
%--------------CHRONIC WHISKER STIM kir2.1 patch dataset  distrubiton ----
if strcmp(PATCH_CONDITION_STRING, 'vsp')
  Gkirbar = mu_stim + sigma_stim*randn(1,tot_cell_number);
end
...
%% %%%%%%%%%%%%%%%% Initial conditions %%%%%%%%%%%%%%%%%%%%%%%%%
Vm_in =  1*ones(1,tot_cell_number);
Vm_in(1:45) = -30; %[mV]
Vm_in(46:75) = -42.5; %[mV]
X0 = Vm_in;
...
cells(PAECs) = [];
cECinds = cells;
plot(tplot, Vm_post(:,cells(K_stim_cell_indx)),'color',[1.0 0.0 0.0],'linewidth',1), grid on, 
xlabel('t(s)'),ylabel('V_m(mV)'), hold on
plot(tplot, Vm_post(:,PAECs),'b','linewidth',2), grid on, 
ylim([-60 -20]);
hold on;
xlim([0, 80])
ax = gca;
...
% plotting dilation hill function on the other axis
fig2 = figure;
FigName = 'Fold Increase in Diameter';
fig2.Name = FigName;
fig2.Renderer = 'painters';
fig2.Units = 'inches';
fig2.Position = [1 1 5 4.5];
hold all
D_coeff = 0.53; %maximal % dilation;
hill_D = 4; % hill coefficient
KA = 55; % half_max dialtion Vm;
Vm_Dilation = abs(mean(Vm_post(:,PAECs),2));
Vm_Dilation=Vm_Dilation';
D = (1 + (D_coeff.*((Vm_Dilation.^hill_D)./(KA^hill_D + Vm_Dilation.^hill_D))));
plot(tplot, D,'k','linewidth',2), grid on, ylabel('Fold Increase in Diameter')
